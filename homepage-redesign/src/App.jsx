  ...
  return <div className="site-shell">
    <header className="site-header" id="top">
      ...
      <nav className={menuOpen ? 'nav open' : 'nav'} aria-label="Main navigation">
        <a href="#intelligence" onClick={() => setMenuOpen(false)}>The platform</a><a href="#solutions" onClick={() => setMenuOpen(false)}>Solutions</a><a href="#method" onClick={() => setMenuOpen(false)}>Methodology</a><a href="https://delquant.com/developers/">Developers</a><a href="#contact" onClick={() => setMenuOpen(false)}>Contact</a>
      </nav>
    ...
    <main>
      <section className="hero" aria-labelledby="hero-title">
        <div className="hero-art" aria-hidden="true"><img src="/assets/hero-intelligence.png" alt=""/></div>
      ...
      <section className="intelligence section-pad" id="intelligence" aria-labelledby="intelligence-title"><div className="intelligence-intro reveal"><div className="eyebrow">FROM DATA TO DECISIONS</div><h2 id="intelligence-title">PLAYER<br/>INTELLIGENCE<span className="period">.</span></h2><p>Every player profile blends proprietary models, league context, and historical research into a clear, actionable view.</p><div className="intelligence-menu" role="tablist" aria-label="Player intelligence views">{tabs.map(tab=><button role="tab" aria-selected={active===tab} className={active===tab ? 'selected':''} key={tab} onClick={()=>setActive(tab)}>{tab}<Arrow/></button>)}</div></div><div className="player-art"><img src="/assets/player-portrait.png" alt="Fictional basketball player in a dark jersey"/><div className="player-identity"><span>PLAYER PROFILE / 001</span><strong>THE<br/>OFFENSIVE<br/>HUB</strong></div></div><div className="dashboard reveal"><div className="dashboard-head"><div><span>DELQUANT / PLAYER INTELLIGENCE</span><strong>A complete player picture.</strong></div><small>● INTERACTIVE DEMO</small></div><div className="dashboard-tabs" role="tablist" aria-label="Player dashboard data">{tabs.map(tab=><button role="tab" aria-selected={active===tab} className={active===tab ? 'selected':''} key={tab} onClick={()=>setActive(tab)}>{tab}</button>)}</div><div className="dashboard-body"><DataPanel active={active}/></div></div><div className="intelligence-quote">“Context turns talent<br/>into edge.”<span>— DELQUANT PRINCIPLE</span></div></section>
      <section className="solutions section-pad" id="solutions" aria-labelledby="solutions-title"><div className="section-heading reveal"><div><div className="eyebrow">ONE ENGINE. MANY DECISIONS.</div><h2 id="solutions-title">BUILT FOR<br/>DECISION MAKERS<span className="period">.</span></h2></div><p>Different goals. A common advantage. DelQuant delivers the player intelligence infrastructure powering better decisions across the basketball ecosystem.</p></div><div className="outcome-grid">{outcomes.map(item=><article className="outcome reveal" key={item.title}><div className={`outcome-visual ${item.visual}`}>{item.visual==='team' && <img src="/assets/team-room.png" alt="Basketball strategy room with an illuminated tactics board"/>}{item.visual==='market' && <div className="market-visual"><span>SAMPLE / MARKET SIGNALS</span><div><b>A</b><b>+4.5</b><b>225.5</b><b>−210</b></div><div><b>B</b><b>−4.5</b><b>220.5</b><b>+175</b></div><div className="market-bars">{[17,25,31,22,39,43,35,49,57,54,68,62,76,72,83].map((height,i)=><i key={i} style={{height: `${height}%`}}/>)}</div></div>}{item.visual==='hoop' && <img src="/assets/hoop-editorial.png" alt="Amber lit hoop in an empty basketball arena"/>}</div><div className="eyebrow">{item.category}</div><h3>{item.title}</h3><p>{item.copy}</p><a href={item.href}>Explore solution <Arrow/></a></article>)}</div></section>
